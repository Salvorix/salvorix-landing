...
const TIERS: Tier[] = [
	{
		...
		cta: 'Start a pilot',
		href: 'https://salvorix-app.heulaulab.xyz/sign-up?plan=pit',
	},
	{
		name: 'Bench',
		price: 'Rp 12 jt',
		...
		href: 'https://salvorix-app.heulaulab.xyz/sign-up?plan=bench',
		highlight: true,
	},
	{
		name: 'Mill',
		...
		href: 'mailto:ops@salvorix-app.heulaulab.xyz?subject=Mill%20plan',
	},
];
		...
		>
			<div className='mx-auto px-6 py-24 md:py-32 max-w-[1400px]'>
				<div className='max-w-[60ch]'>
					<h2 className='font-semibold text-[clamp(2rem,4vw,3.25rem)] text-ink-900 dark:text-ink-50 leading-[1.05] tracking-[-0.02em]'>
						Priced per site.
						<br />
						Not per seat.
					</h2>
					<p className='mt-5 text-[17px] text-ink-600 dark:text-ink-300 leading-[1.55]'>
						A pit foreman with a clipboard should not be the line item that
						blocks the rollout. Unlimited operators, unlimited viewers. We meter
						on the thing that costs us money: data and storage.
					</p>
				</div>

				<div className='gap-4 grid grid-cols-1 md:grid-cols-3 mt-14'>
					{TIERS.map((t) => (
						<article
							key={t.name}
						...
						>
							<div className='flex justify-between items-baseline'>
								...
								{t.highlight && (
									<span className='text-hazard stamped'>most picked</span>
								)}
							</div>
							<p className='opacity-80 mt-2 max-w-[36ch] text-sm'>{t.fit}</p>

							<div className='flex items-baseline gap-2 mt-8'>
								<span className='font-semibold text-4xl tracking-tight ticker'>
									{t.price}
								</span>
								<span className='opacity-70 text-sm'>{t.unit}</span>
							</div>
				...
				<p className='mt-8 text-ink-500 dark:text-ink-400 text-xs'>
